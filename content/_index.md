---
title: Markdown 文档
bookToc: false
type: docs
---

# Markdown 学习笔记

<a target="_blank" href="https://gohugo.io/"  title="Hugo"><img src="https://cdn.xalaok.top/badge/hugo.svg" alt=""></a>
<a target="_blank" href="https://vercel.com/"  title="托管于 Vercel"><img src="https://cdn.xalaok.top/badge/vercel.svg" alt=""></a>
<a target="_blank" href="https://github.com/"  title="网站源码由 Github 提供存储仓库"><img src="https://cdn.xalaok.top/badge/github.svg" alt=""></a>
<a target="_blank" href="https://creativecommons.org/licenses/by-nc-sa/4.0/"  title="无特殊声明默认采用 CC BY-NC-SA 4.0 协议"><img src="https://cdn.xalaok.top/badge/ccbyncsa.svg" alt=""></a>

<img src="/md.webp" height=10 style="align:center">

一份 Markdown 学习笔记，可以快速上手 Markdown 写作。

主要内容：

- Markdown 语法
- Markdown 写作规范

原作者：[keatonlao](https://keatonlao.gitee.io/a-study-note-for-markdown/) 最近更新时间大约 3 年前

修改内容：

- 统一使用减号 `-` 来表示无序列表
- 修改部分关于时间的用词
- 统一使用英文文件名
- 根据“Markdown 写作规范”里提及的相关国家标准与行业标准修正部分问题
- 调整文件树结构
- 删除多余空行、举例
- 修改 Front Matter 及文章内短代码以适配本主题
- 文字格式从简，尽量纯文本，重内容
- 用 Markdown 语法替换部分不必使用 HTML 语法的地方
- 删除部分与 Markdown 本身主题偏离的文档
- 替换部分国家标准失效的链接
- Typora 性质变更

---

> ➀ 本笔记含有许多网络文章，感谢其作者们！  
> ➁ 如果发现文档有问题请 [Report issues](https://github.com/Xalaok/md-docs/issues)。

---

This work is licensed under [ CC BY-NC-SA 4.0 ![img](data:image/png;base64,)![img](data:image/png;base64,)![img](data:image/png;base64,)![img](data:image/png;base64,)](https://creativecommons.org/licenses/by-nc-sa/4.0)

<span id="timeDate">载入天数...</span><span id="times">载入时分秒...</span>
<script>
    var now = new Date();
    function createtime(){
        // 当前时间
        now.setTime(now.getTime()+250);
        var grt= new Date("2023/11/21 20:00:00"); //网站诞生时间

        days = (now - grt ) / 1000 / 60 / 60 / 24;
        dnum = Math.floor(days);
        hours = (now - grt ) / 1000 / 60 / 60 - (24 * dnum);
        hnum = Math.floor(hours);
        if(String(hnum).length ==1 ){hnum = "0" + hnum;}
        minutes = (now - grt ) / 1000 /60 - (24 * 60 * dnum) - (60 * hnum);
        mnum = Math.floor(minutes);
        if(String(mnum).length ==1 ){mnum = "0" + mnum;}
        seconds = (now - grt ) / 1000 - (24 * 60 * 60 * dnum) - (60 * 60 * hnum) - (60 * mnum);
        snum = Math.round(seconds);
        if(String(snum).length ==1 ){snum = "0" + snum;}

        document.getElementById("timeDate").innerHTML = "本站已稳定运行 "+dnum+" 天 ";
        document.getElementById("times").innerHTML = hnum + " 小时 " + mnum + " 分 " + snum + " 秒";
    }
    setInterval("createtime()",250); 
</script>